...
export default function BookSession({ session, onDone }: BookSessionProps) {
  ...
  // useEffect is used to open the Modal via its exposed
  //`open` method when the component is mounted
  useEffect(() => {
    if (modal.current) {
      modal.current.open();
    }
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    console.log("Form data", data); // would normally be sent to a server, together with session data
